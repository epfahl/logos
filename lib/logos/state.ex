defmodule Logos.State do
  ...
  The state holds a counter for the number of created logical variables, and a map from variables
  to their subitution values. All low-level interactions with a state take place through the functions in this module.
  """
  ...
  @doc """
  Fetch the value assocaited with the given logical `var`. This returns `{:ok, value}` if `var` is
  present, and `:error` otherwise.
  """
  ...
  @doc """
  Retrieve the value associated with a term by traversing the relationships between variables in
  the state, and stopping when the value is a list or constant.
  """
