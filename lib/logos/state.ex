defmodule Logos.State do
  ...
  The state holds a counter for the number of created logical variables, and a map from
  variables to their subitutions. All low-level interactions with a state take place
  ...
  @doc """
  Fetch the value assocaited with the given logical `var`. This returns `{:ok, value}`
  if `var` is present, and `:error` otherwise.
  """
  ...
  """
  def extend(%S{sub: sub} = state, key, value), do: %{state | sub: Map.put(sub, key, value)}
  ...
  @doc """
  Retrieve the value associated with a term by traversing the relationships between
  variables in the state, and stopping when the value is a list or constant.
  """
  ...
  @doc """
  Deeply traverse the state by walking both variables and lists that may contain
  variables.
  """
